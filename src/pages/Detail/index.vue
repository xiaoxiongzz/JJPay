...
export default {
  ...
  mounted() {
    this.getData(this.$route.params.skuid);
  },
  computed: {
    ...mapGetters("detail", ["skuInfo", "spuSaleAttrList", "categoryView"]),
    ...mapState("detail", ["goodsInfo"]),
  },
  methods: {
    ...mapActions("detail", { getData: "getGoodsInfo" }), 
    //产品属性值高亮显示(属性列表，属性值)
    //遍历相当于排他思想，先清空所有的高亮显示
    ...
    changeNum(event) {
      ...
      //用户输入进来的 *1，为NaN，则为非法，skuNum=1//输入的小于1，则为非法,非法默认为1
      if (isNaN(value) || value < 1) {
        this.skuNum = 1;
      }
      //输入常规数字【大于1，整数】，判断是否是整数
      else {
