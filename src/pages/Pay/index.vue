...
export default {
  ...
  methods: {
    //获取支付信息
    async getPayInfo() {
      let result = await this.$api.reqSubmitOrderInfo(this.orderId);
      if (result.code == 200) {
    ...
    async open() {
      ...
      this.$alert(`<img src=${url} />`, "请使用微信支付", {
        ...
        cancelButtonText: "支付遇到的问题", //取消text
        confirmButtonText: "支付成功", //确定text
        showCancelButton: true, //取消按钮显示
        center: true, //剧中
        showClose: false,
        beforeClose: (type, instance, done) => {
          // type - cancel取消按钮(左）--confirm确认按钮 (右)
          //instance - 当前vm实例
          //done 关闭msg
          if (type == "cancel") {
