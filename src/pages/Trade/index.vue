  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      >
        <span
          ...
          @click="changeAddressInfo(address)"
          >{{ address.consignee }}</span
        >
        <p @click="changeAddressInfo(address)">
    ...
    <div class="trade">
      ...
      <div class="receiveInfo">
        寄送至:
        <span>{{ goAddressInfo.fullAddress }}</span>
        收货人：<span>{{ goAddressInfo.consignee }}</span>
        <span>{{ goAddressInfo.phoneNum }}</span>
      </div>
    </div>
  ...
  computed: {
    ...
    totalPrice() {
      ...
      this.detailArrayList.forEach((item) => {
        total += item.orderPrice * item.skuNum;
      });
      return total;
    },
    //商品总数
    ...
    //配送地址
    goAddressInfo() {
      return this.addressInfo.find((item) => item.isDefault == 1) || {}; //{}防止数据没有返回时产生报错
    },
  },
  methods: {
    //修改默认地址显示
    ...
    async submitOrder() {
      ...
      let data = {
        consignee: this.goAddressInfo.consignee, //收货名
        consigneeTel: this.goAddressInfo.phoneNum, //收件人手机号
        deliveryAddress: this.goAddressInfo.fullAddress, //收件人收件地址
        paymentWay: "ONLINE", //支付方式
        orderComment: this.text, //买家留言
